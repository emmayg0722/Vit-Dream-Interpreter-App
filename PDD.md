...
All six M1 tasks (6.2) are authored. What's outstanding before M1 can be called done: the first `xcodebuild` build + test run on macOS (this was all written in a Linux agent session with no Swift toolchain — see 5.2), and a visual pass comparing the running app against `design/prototype.jsx`.
...
| 2026-07-17 | TASK-002 through TASK-006: design system (tokens, aurora, glass, orb, ring, tone chip), `ReadingDTO` + mosquito fixture + decoding tests, capture screen with draft persistence, reading screen rendering the fixture; sample dream wired end-to-end offline (FR-011). M1 fully authored, macOS build/test verification still outstanding | M1 foundation complete pending macOS verification | `Core/DesignSystem/*`, `Core/Models/ReadingDTO.swift`, `Core/Fixtures/SampleDream.swift`, `Features/Capture/*`, `Features/Reading/*`, test targets |
